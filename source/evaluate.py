...
from paths import REPO_DIR, ASSET_DATASET, get_data_filepath,  OUTPUT_DIR
from transformers import AutoTokenizer, AutoModelForSeq2SeqLM
from paths import ASSET_DATASET

DEFAULT_METRICS = ['bleu', 'sari', 'fkgl', 'sent_bleu', 'f1_token', 'sari_by_operation']

def evaluate_on_dataset(features_kwargs, model_dirname, eval_dataset, project_name):
    ...
    for i in range(len(pred_filepath)):
        ...
        print("SARI: {:.2f} \t BLEU: {:.2f} \t FKGL: {:.2f} \t SENT_BLEU: {:.2f} \t F1 {:.2f}  \t SARI_ADD {:.2f} \t SARI_KEEP {:.2f} \t SARI_DELETE {:.2f}".format(scores['sari'], scores['bleu'], scores['fkgl'], scores['sent_bleu'],  scores['f1_token'],  scores['sari_add'], scores['sari_keep'], scores['sari_del'])) 
        print(scores)
        wandb.log(scores)
        return  scores['sari']
    
def evaluate_on_asset(features_kwargs, model_dirname, eval_dataset): 
...
def calculate_eval_sentence(dataset, test_dataset, predictions):
    ...
    for i in range(0,len(predictions)):  
        ...
        references = []
        refs = []
        if  'simp.0' in test_dataset.column_names:
