...
wandb_kwargs = {"project": "hyperparameter_tuning_local"} 
...
def objective(trial):
    ...
    training_args = Seq2SeqTrainingArguments( 
        ...
        learning_rate=  trial.suggest_float('learning_rate', low=0.000004, high=0.0001), # , step=0.0005) # , log=False),             
        per_device_train_batch_size= 3, # trial.suggest_categorical('batch_size', [6, 8, 12, 18]),       
        per_device_eval_batch_size= 3, # trial.suggest_categorical('batch_size', [6, 8, 12, 18]),  
        optim="adamw_hf",  
...
if __name__ == '__main__':
    ...
    study = optuna.create_study( direction='minimize',pruner=optuna.pruners.ThresholdPruner(upper=2.0))
    study.optimize(objective, n_trials=2)
    # time.sleep(10)
    # study.optimize(objective, n_trials=1,callbacks=[wandbc],  gc_after_trial=True)
